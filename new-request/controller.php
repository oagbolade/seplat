<?php
	...
	$Tag = new Tag(URL);
	$head = $Tag->createHead("Membership Application Portal | New Request ", "nav-md home-page", ['css' => ['css/nprogress.css']]);
	...
	$DbHandle->setTable('service');
	$categoryOption = "";
	if ($services = $DbHandle->iRetrieveData(__LINE__)) {
		foreach ($services as $service) {
			$categoryOption .= "<option value='{$service['service']}'>{$service['service']}</option>";
		}
	}
